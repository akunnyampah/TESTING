...
_DH_PARAMS = [
    # (joint, a_mm, alpha_deg, d_mm, note)
    ("J1",   0.00,    0.0,   110.50, ""),
    ("J2",  23.42,  -90.0,     0.00, ""),
...
def _show_dh_dialog() -> None:
    from waldo_commander.state import robot_state  # avoid circular import at module level
    ...
    with ui.dialog() as dialog, ui.card().classes("min-w-[500px]"):
        with ui.row().classes("w-full items-center justify-between"):
            ui.label("DH Parameters — PAROL6").classes("text-base font-semibold")
    ...
    dialog.on("hide", lambda: timer.deactivate())
    timer.activate()
    dialog.open()
...
def create_ros2_tab_content() -> dict:
    ...
    with ui.column().classes("w-full gap-3 p-3"):
        ...
        ui.separator()
        ui.button(
            "DH Parameters", icon="table_chart", on_click=_show_dh_dialog
        ).classes("w-full")
